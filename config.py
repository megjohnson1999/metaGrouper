...
class MetaGrouperConfig:
    ...
    def validate_parameters(self, params: Dict[str, Any]) -> List[str]:
        ...
        if 'permutations' in params:
            ...
            if perms < min_perms or perms > max_perms:
                errors.append(f"Permutations must be between {min_perms} and {max_perms}")
    ...
    def optimize_for_system(self, memory_gb: float, cpu_cores: int) -> Dict[str, Any]:
        ...
        if cpu_cores >= 8:
            optimized['permutations'] = min(optimized.get('permutations', 999) * 2, 9999)
...
def estimate_resource_requirements(n_samples: int, avg_reads_per_sample: int, 
                                 kmer_size: int) -> Dict[str, Any]:
    """Estimate computational resource requirements."""
    
    # Rough estimates based on typical performance
    
    # Memory estimation
    unique_kmers_per_sample = min(avg_reads_per_sample * (150 - kmer_size + 1), 
                                 4 ** kmer_size)  # Theoretical max
    
    memory_per_sample_mb = unique_kmers_per_sample * 8 / 1024 / 1024  # 8 bytes per k-mer
    ...
    estimated_runtime_minutes = (base_time_per_sample * n_samples * 
                                kmer_factor * reads_factor) / 60
    ...
    return {
        ...
        'recommended_preset': (
            'quick' if n_samples > 100 or total_memory_mb > 8192 else
            'high_quality' if n_samples < 20 else
            'default'
        )
    }
...
if __name__ == '__main__':
    ...
    parser.add_argument('--estimate-resources', nargs=3, type=int,
    ...
    if args.estimate_resources:
        ...
        estimates = estimate_resource_requirements(n_samples, reads_per_sample, kmer_size)
