...
def generate_realistic_metadata():
    ...
    metadata = {
        ...
        'treatment_group': ['control', 'control', 'treatment', 'treatment'],  # Clear grouping
        ...
        'sequencing_batch': ['batch1', 'batch1', 'batch2', 'batch2'],  # Technical grouping
...
def run_comprehensive_example():
    ...
    cmd = [
        ...
        "--permutations", "199",  # Faster for demo
        "--cluster-range", "2", "4",
        "--assembly-tools", "megahit", "spades",
        "--similarity-threshold", "0.25",  # More stringent
        "--min-group-size", "2",
        "--max-group-size", "6",
        "--verbose"
    ]
...
def demonstrate_assembly_tools():
    ...
    for tools, output_suffix in tool_configs:
        ...
        cmd = [
            sys.executable, "metagrouper.py",
            "example_data",
            "--output", f"assembly_test_{output_suffix}",
            "--kmer-size", "15",
            "--max-reads", "300",  # Faster
            "--metadata", str(metadata_file),
            "--assembly-tools"] + tools + [
            "--permutations", "99",  # Very fast
        ...
        try:
            ...
            scripts_dir = Path(f"assembly_test_{output_suffix}") / "assembly_recommendations"
...
def demonstrate_threshold_sensitivity():
    ...
    for threshold in thresholds:
        ...
        cmd = [
            sys.executable, "metagrouper.py",
            "example_data",
            "--output", f"threshold_test_{threshold:.2f}".replace(".", "p"),
            "--kmer-size", "15",
            "--max-reads", "300",
            "--metadata", str(metadata_file),
        ...
        try:
            ...
            strategy_file = Path(f"threshold_test_{threshold:.2f}".replace(".", "p")) / "assembly_recommendations" / "assembly_strategy.md"
            if strategy_file.exists():
                with open(strategy_file, 'r') as f:
                    content = f.read()
                    if "Overall Strategy:" in content:
                        strategy_line = [line for line in content.split('\n') if 'Overall Strategy:' in line][0]
