...
try:
    from metadata_analyzer import MetadataAnalyzer, MetadataVisualizer, generate_summary_report
...
try:
    from assembly_recommender import AssemblyRecommender, save_recommendations, visualize_assembly_strategy
...
class KmerProfiler:
    ...
    def profile_sample(self, filepath: str, sample_name: str) -> Dict[str, int]:
        ...
        normalized_profile = {kmer: count / total_count 
...
class Visualizer:
    ...
    def plot_mds(self, mds_result: np.ndarray, output_path: str):
        ...
        # Add sample labels
        for i, sample in enumerate(self.sample_names):
            plt.annotate(sample, (mds_result[i, 0], mds_result[i, 1]), 
                        xytext=(5, 5), textcoords='offset points')
...
def save_results(profiles: Dict, distance_matrix: np.ndarray, 
                sample_names: List[str], output_dir: str):
...
def main():
    ...
    parser.add_argument('-m', '--metadata', 
                       help='Metadata file (CSV/TSV) for Phase 2 analysis')
    ...
    visualizer.plot_distance_heatmap(distance_matrix, 
                                   output_path / 'distance_heatmap.png')
    visualizer.plot_pca(pca_result, pca, output_path / 'pca_plot.png')
    visualizer.plot_mds(mds_result, output_path / 'mds_plot.png')
    
    # Save results
    save_results(profiler.profiles, distance_matrix, 
                profiler.sample_names, args.output)
    ...
    if args.metadata and PHASE2_AVAILABLE:
        ...
        try:
            ...
            meta_visualizer = MetadataVisualizer(profiler.sample_names, meta_analyzer.metadata)
            ...
            for method, method_results in cluster_results.items():
                if 'optimal' in method_results:
                    ...
                    meta_visualizer.plot_clustering_results(
                        pca_result, optimal['labels'], method, 
                        optimal['n_clusters'],
            ...
            generate_summary_report(
                metadata_results_df, cluster_results,
    ...
    if PHASE3_AVAILABLE:
        ...
        try:
            ...
            assembly_recommendation = recommender.generate_recommendations(
                ...
                metadata=meta_analyzer.metadata if 'meta_analyzer' in locals() else None
            )
            
            # Filter assembly commands to requested tools
            filtered_commands = {tool: commands for tool, commands in assembly_recommendation.assembly_commands.items() 
                               if tool in tools}
