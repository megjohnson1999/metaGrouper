...
For more information, see the tutorial: https://github.com/user/metagrouper/blob/main/TUTORIAL.md
    ...
    general_group.add_argument(
        "--version", action="version", version="MetaGrouper 1.0.0"
    )
    ...
    metadata_group.add_argument(
        "-m", "--metadata", help="Metadata file (CSV/TSV) for statistical analysis"
    )
    metadata_group.add_argument(
        "--sample-id-column",
        default="sample_id",
...
def validate_arguments(args) -> List[str]:
    ...
    if not 0.0 <= args.similarity_threshold <= 1.0:
        errors.append(
            f"Similarity threshold must be between 0.0 and 1.0: {args.similarity_threshold}"
        )
    ...
    if args.permutations < 99:
        errors.append(
            f"Number of permutations must be at least 99: {args.permutations}"
        )
...
def print_final_summary(args, start_time: float, fastq_files: List[str], success: bool):
    ...
    print(
        f"⏱️  Total runtime: {duration:.1f}s" if duration < 60 else f"{duration/60:.1f}m"
    )
