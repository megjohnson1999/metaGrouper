...
class PermanovaAnalyzer:
    ...
    def _calculate_sum_of_squares(self, distance_matrix: np.ndarray, 
    ...
    def permanova_test(self, metadata_variable: np.ndarray, 
        ...
        valid_distance_matrix = self.distance_matrix[np.ix_(valid_indices, valid_indices)]
        valid_groups = metadata_variable[valid_indices]
        
        # Calculate observed F-statistic
        within_ss, total_ss = self._calculate_sum_of_squares(valid_distance_matrix, valid_groups)
...
class MetadataAnalyzer:
    ...
    def load_metadata(self, metadata_file: str, sample_id_column: str = 'sample_id'):
        ...
        logging.info(f"Loaded metadata for {len(self.metadata)} samples with "
        ...
        for variable in variables:
            ...
            if var_data.dtype == 'object':
                ...
                if not np.all(np.isnan(var_array)):
                    ...
                    if len(np.unique(valid_values)) > 10:  # Only bin if many unique values
                        quantiles = np.percentile(valid_values, [33, 67])
                        var_binned = np.full_like(var_array, np.nan)
                        var_binned[~np.isnan(var_array)] = np.digitize(valid_values, quantiles)
                        var_array = var_binned
            
            # Run PERMANOVA
            result = permanova.permanova_test(var_array, n_permutations)
            result['variable'] = variable
            result['variable_type'] = 'categorical' if self.metadata[variable].dtype == 'object' else 'numerical'
    ...
    def identify_clusters(self, n_clusters_range: Tuple[int, int] = (2, 8),
    ...
    def compare_clustering_with_metadata(self, clustering_labels: np.ndarray,
        ...
        for variable in variables:
            ...
            valid_indices = [i for i, name in enumerate(self.sample_names) 
                           if name in var_data.index]
            ...
            results.append({
                ...
                'variable_type': 'categorical' if var_data.dtype == 'object' else 'numerical',
...
class MetadataVisualizer:
    ...
    def plot_variable_importance(self, results_df: pd.DataFrame, output_path: str):
        ...
        if valid_results.empty:
            plt.text(0.5, 0.5, 'No valid results to display', 
        ...
        for i, (_, row) in enumerate(valid_results.iterrows()):
            plt.text(row['r_squared'] + 0.01, i, f"{row['r_squared']:.3f}", 
    ...
    def plot_samples_by_variable(self, pca_result: np.ndarray, variable: str, 
                               output_path: str, pca_obj: Optional[PCA] = None):
        ...
        if not np.any(valid_mask):
            plt.text(0.5, 0.5, f'No valid data for variable: {variable}', 
                    ha='center', va='center', transform=plt.gca().transAxes)
            ...
            for i, value in enumerate(unique_values):
                ...
                if np.any(mask):
                    plt.scatter(pca_result[mask, 0], pca_result[mask, 1], 
                              c=[colors[i]], label=str(value), s=100, alpha=0.7)
            
            plt.legend(bbox_to_anchor=(1.05, 1), loc='upper left')
        else:
            # Numerical variable
            valid_data = var_data[valid_mask]
            scatter = plt.scatter(pca_result[valid_mask, 0], pca_result[valid_mask, 1], 
                                c=valid_data, cmap='viridis', s=100, alpha=0.7)
...
def generate_summary_report(results_df: pd.DataFrame, cluster_results: Dict,
                          output_path: str):
    ...
    with open(output_path, 'w') as f:
        ...
        f.write("Variables ranked by proportion of variation explained (R-squared):\n\n")
        ...
        if not results_df.empty:
            ...
            for _, row in valid_results.iterrows():
                ...
                f.write(f"- **{row['variable']}**: R² = {row['r_squared']:.3f}, "
        ...
        for method, method_results in cluster_results.items():
            ...
            if 'optimal' in method_results:
                ...
                f.write(f"- **Silhouette score**: {optimal['silhouette_score']:.3f}\n\n")
                
                # Show silhouette scores for different k values
                f.write("Silhouette scores for different k values:\n")
                for k in sorted([k for k in method_results.keys() if isinstance(k, int)]):
        ...
        if not results_df.empty:
            ...
            if not valid_results.empty:
                ...
                f.write(f"**{top_variable['variable']}** explains the most variation "
                       f"({top_variable['r_squared']:.1%}) in sample similarities.\n\n")
                
                if top_variable['p_value'] < 0.05:
                    f.write("This variable shows statistically significant association "
                           "with sample composition (p < 0.05).\n\n")
                    f.write("**Recommendation**: Consider grouping samples by this variable "
                           "for co-assembly.\n\n")
                else:
                    f.write("This association is not statistically significant (p ≥ 0.05).\n\n")
                    f.write("**Recommendation**: Consider individual sample assembly or "
                           "global co-assembly.\n\n")
        
        # Add interpretation guide
        f.write("## Interpretation Guide\n\n")
        f.write("- **R-squared**: Proportion of variation in sample composition explained by the variable\n")
        f.write("- **p-value**: Statistical significance (< 0.05 is typically significant)\n")
        f.write("- **Silhouette score**: Quality of clustering (higher is better, > 0.5 is good)\n")
